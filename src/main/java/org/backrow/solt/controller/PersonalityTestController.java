...
public class PersonalityTestController {
    ...
    @Operation(summary = "유형 검사 정보 가져오기", description = "유형검사 ID를 통해 질문, 답변정보를 가져옵니다")
    ...
    @Operation(summary="유형검사 전체 결과 가져오기", description = "모든 유형 결과 리스트를 가져옵니다")
    @GetMapping("/getAllResults")
    public List<ResultAllDTO> getAllResults() {
        return personalityTestService.getAllResults();
    }

    @Operation(summary = "유형검사 결과  상세조회", description = "ID를 통해 특정 유형검사결과를 상세조회합니다.")
