...
import org.backrow.solt.service.PersonalityTestLogService;
...
public class PersonalityTestLogController {
    ...
    @Operation(summary = "유형검사결과 저장", description = "완료한 유형검사결과를 저장합니다")
    @PostMapping
    ...
    @Operation(summary = "유형검사결과 목록 조회", description = "사용자가 완료한 유형검사결과 목록을 조회합니다")
    @GetMapping("/list")
...
//    @Operation(summary = "유형검사결과 상세 조회", description = "ID를 통해 특정 유형검사결과 조회합니다")
//    @GetMapping("/{id}")
//    public ResponseEntity<SurveyLogViewDTO> getLog(@PathVariable Long surveyLogId) {
//        return null;
//    }

    @Operation(summary = "유형검사결과  삭제", description = "ID를 통해 특정 유형검사결과를 삭제합니다.")
    @DeleteMapping("/{id}")
